  ...
  try {
    ...
    if (!paymentTxId) {
      ...
      return NextResponse.json({
        success: true,
        paymentTx: Buffer.from(paymentTx.serialize({ requireAllSignatures: false })).toString(
          'base64',
        ),
        blockhash,
    ...
    if (liquidityDelta.lte(new BN(0))) {
      return NextResponse.json(
        { success: false, error: 'Liquidity delta <= 0, cannot create pool' },
        { status: 400 },
      );
    }
    ...
    const poolId = derivePoolAddress(config, new PublicKey(tokenAMint), new PublicKey(tokenBMint));
