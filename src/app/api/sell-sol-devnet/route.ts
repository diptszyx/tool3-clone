...
export async function POST(request: Request) {
  try {
    const { walletPublicKey, tokenAmount, tokenMint, solAmount } =
      (await request.json()) as SellSolRequest;
    ...
    if (!isValidPublicKey(walletPublicKey) || !isValidPublicKey(tokenMint)) {
      return NextResponse.json({ error: 'Invalid public key(s)' }, { status: 400 });
    }
