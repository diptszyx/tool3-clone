...
export async function POST(request: Request) {
  try {
    ...
    const transferInstruction = transaction.instructions.find(
      (ix) => ix.programId.equals(TOKEN_PROGRAM_ID) || ix.programId.equals(SystemProgram.programId),
    );
    ...
    if (tokenMint === NATIVE_SOL) {
      isToAdmin = transferInstruction.keys.some((key) => key.pubkey.equals(adminPublicKey));
    } else {
      const adminTokenAccount = await getAssociatedTokenAddress(
        new PublicKey(tokenMint),
        adminPublicKey,
      );
      isToAdmin = transferInstruction.keys.some((key) => key.pubkey.equals(adminTokenAccount));
    }
    ...
    const txSignature = await connectionMainnet.sendRawTransaction(transaction.serialize(), {
      skipPreflight: false,
      preflightCommitment: 'confirmed',
      maxRetries: 3,
    });

    const confirmation = await connectionMainnet.confirmTransaction(txSignature, 'confirmed');
