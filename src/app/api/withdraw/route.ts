...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      const userToken0AccountInfo = await connectionDevnet.getAccountInfo(userToken0Account);
      if (!userToken0AccountInfo) {
        console.log('Creating user token 0 account:', userToken0Account.toBase58());
        transaction.add(
          createAssociatedTokenAccountInstruction(
            walletPublicKey,
            userToken0Account,
      ...
      console.log('Error checking user token 0 account, creating new one:', error);
      transaction.add(
        createAssociatedTokenAccountInstruction(
          walletPublicKey,
          userToken0Account,
    ...
    try {
      const userToken1AccountInfo = await connectionDevnet.getAccountInfo(userToken1Account);
      if (!userToken1AccountInfo) {
        console.log('Creating user token 1 account:', userToken1Account.toBase58());
        transaction.add(
          createAssociatedTokenAccountInstruction(
            walletPublicKey,
            userToken1Account,
      ...
      console.log('Error checking user token 1 account, creating new one:', error);
      transaction.add(
        createAssociatedTokenAccountInstruction(
          walletPublicKey,
          userToken1Account,
    ...
    if (error instanceof SendTransactionError) {
      ...
      errorMessage = `Transaction failed: ${error.message}. Logs: ${JSON.stringify(logs)}`;
