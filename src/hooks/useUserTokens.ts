...
const loadFallbackTokenList = async (forceRefresh = false): Promise<Record<string, Token>> => {
  if (!forceRefresh && fallbackTokenList && Date.now() - fallbackTokenList.timestamp < CACHE_TTL) {
    ...
    try {
      ...
      const formattedTokens: UserToken[] = enrichedAssets
        ...
        .map((asset) => {
          const mint = asset.id;
          const balance =
            (asset.token_info?.balance || 0) / Math.pow(10, asset.token_info?.decimals || 0);
          return {
            address: mint,
        ...
        .filter(
          (token: UserToken) =>
            !excludeToken ||
            (!excludeToken.includes(token.address) &&
      ...
      const allTokens =
        !excludeToken || !excludeToken.includes(NATIVE_SOL)
          ? [solToken, ...filteredFormattedTokens]
          : filteredFormattedTokens;
