...
export default function Withdraw() {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Token withdrawal failed. Please try again.';
      toast.error(message);
    } finally {
      setLoading(false);
    }
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <div className="flex items-center gap-4 mt-1">
                    <FormControl>
                      <Input
                        className="border-gear-gray h-[28px] bg-white text-gray-900 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                        placeholder="0.00"
