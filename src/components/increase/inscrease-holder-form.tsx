...
import { FormData, DEFAULT_TOKEN, TokenInfo, WalletInfo, FORM_SCHEMA } from '@/lib/increase/types';

export default function IncreaseHolderForm() {
  const isMobile = useIsMobile();
  const [tokenModalOpen, setTokenModalOpen] = useState(false);
  const [selectedToken, setSelectedToken] = useState<TokenInfo | null>(DEFAULT_TOKEN);
  const [isLoading, setIsLoading] = useState(false);
  const { publicKey, signTransaction } = useWallet();

  const { wallets, generateWallets, saveWallets, clearWallets } = useWalletManager();
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const { initialTransaction, childTransactions } = await buildAirdropTransactions(
        publicKey,
        fullList,
        connection,
        selectedToken?.id,
      ...
      const updatedWallets = await fetchSolAndTokenBalancesBatched(currentWallets, connection);
  ...
  return (
    <div className="mx-auto my-2 md:p-2">
      <div className="min-h-screen">
        <div
          className={`max-h-[calc(100vh-100px)] overflow-y-auto px-4 ${isMobile ? 'py-2' : 'py-6'}`}
        >
          <div className="max-w-7xl mx-auto px-4 py-6">
            <div className="flex items-center gap-3 mb-4">
              <BarChart3 className="h-8 w-8 text-black" />
              <h1 className="text-3xl font-bold text-gray-900">SOL Increase Holders</h1>
            </div>
            <p className="text-gray-600 mb-6 max-w-4xl">
              Quickly increase the number of new wallet Buy (↑MAKERS) and token holders for a
              specified token by automatically creating new wallet addresses, helping your project
              data stand out on DEX.
            </p>
          </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                  <WalletTable wallets={wallets} selectedToken={selectedToken} />
