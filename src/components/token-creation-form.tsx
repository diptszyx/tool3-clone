...
export const TokenCreationForm = () => {
  ...
  const handleImageUpload =
    tokenType === 'spl' ? splHandleImageUpload : extensionsHandleImageUpload;
  const handleCreateToken =
    tokenType === 'spl' ? splHandleCreateToken : extensionsHandleCreateToken;
    ...
    const subscription = form.watch((value) => {
      if (tokenType === 'spl') {
        splSetTokenData((previousData: SPLTokenData) => {
          ...
          const isShallowEqual = (previous: SPLTokenData, next: SPLTokenData) => {
            return (
        ...
        extensionsSetTokenData((previousData: ExtensionsTokenData) => {
          ...
          // Tránh set state khi dữ liệu không thay đổi để ngăn vòng lặp render
          const isShallowEqual = (previous: ExtensionsTokenData, next: ExtensionsTokenData) => {
            return (
              previous.name === next.name &&
              previous.symbol === next.symbol &&
              previous.decimals === next.decimals &&
  ...
  useEffect(() => {
    ...
    extensionsTokenData.imageUrl,
    splTokenData.imageUrl,
    splSetTokenData,
    extensionsSetTokenData,
  ]);
  ...
  const onSubmit = () => {
    if (tokenType === 'spl') {
      ...
      const hasMetadataExtension =
        selectedExtensions.includes('metadata') || selectedExtensions.includes('metadata-pointer');
  ...
  return (
    <div className={`md:p-3 mx-auto my-2`}>
      ...
      <TokenTypeSelector tokenType={tokenType} onTokenTypeChange={setTokenType} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className={`pt-6 px-1 pb-2 ${!isMobile && 'border-gear'}`}>
              <Form {...form}>
                <form
                  ...
                  className={`space-y-6 ${!isMobile && 'max-h-[calc(100vh-200px)] overflow-y-auto px-2'}`}
                >
                  <TokenBasicFields
          ...
          {tokenType === 'extensions' ? (
            <TokenExtensionsSidebar
              ...
              onToggleExtension={(extensionId: string) =>
                toggleExtension(extensionId, tokenExtensions)
              }
