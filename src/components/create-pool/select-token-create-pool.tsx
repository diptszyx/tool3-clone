...
import TokenSearchModal from '../transfer/select-token-modal';

interface SelectTokenCreatePoolProps {
...
const SelectTokenCreatePool: React.FC<SelectTokenCreatePoolProps> = ({
  selectedToken,
  setSelectedToken,
  onAmountChange,
  title,
  ...
  }, [tokens, selectedToken, setSelectedToken, userHasSelected]);
      ...
      <div className="flex justify-between items-center mb-2 mt-4 h-[40px]">
        <button
          ...
          className={`flex items-center gap-2 text-gray-700 hover:text-purple-900 border-gear-gray px-2 py-1 ml-2 ${
            !selectedToken ? 'cursor-not-allowed' : 'cursor-pointer'
          }`}
...
export default SelectTokenCreatePool;
