...
export default function CreateRaydiumCpmmPool() {
  ...
  const handleNext = async () => {
    try {
      ...
      if (currentStep === 1) {
        ...
        const txId = await sendTokenTransfer(
          dataTokenTransfer.tokenTransferTx,
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm">
              ...
              <a
                ...
                className="text-blue-500 hover:underline"
              >
                {dataCreatePool.poolKeys.poolId.slice(0, 8)}...
                {dataCreatePool.poolKeys.poolId.slice(-8)}
              </a>
            </p>
          </div>,
        );
  ...
  const price =
    Number.parseFloat(form.watch('amountToken1')) / Number.parseFloat(form.watch('amountToken2')) ||
    '';
        ...
        <div className="flex justify-between mb-4">
          {steps.map((step, index) => (
            ...
            >
              ...
              <div
                className={`h-1 mt-1 ${currentStep >= index + 1 ? 'bg-blue-500' : 'bg-gray-200'}`}
              ></div>
            </div>
          ))}
        </div>
          ...
          {currentStep === 3 && (
            <div className="space-y-6 px-1">
              <div className="text-sm">
                <p className="mt-2">
                  <strong>Token Pair:</strong> {selectedToken1?.symbol || 'UNKNOW'} /{' '}
                  {selectedToken2?.symbol || 'UNKNOW'}
                </p>
                <p className="mt-2">
                  <strong>Amount A:</strong> {form.watch('amountToken1')}{' '}
                </p>
                <p className="mt-2">
                  <strong>Amount B:</strong> {form.watch('amountToken2')}{' '}
                </p>
                <p className="mt-2">
                  <strong>Initial Price:</strong> {price} {selectedToken1?.symbol || 'UNKNOW'}/
                  {selectedToken2?.symbol || 'UNKNOW'}
                </p>
                <p className="mt-2">
              ...
              <div className="text-sm text-gray-500">
                ...
                <p className="mt-2">
                  ...
                  <a
                    ...
                    className="text-blue-500 hover:underline"
                  >
                    {poolResult?.poolKeys.poolId.slice(0, 8)}...
                    {poolResult?.poolKeys.poolId.slice(-8)}
                  </a>
                </p>
                <p className="mt-2">
                  Click Create Another Pool to start a new pool creation process.
                </p>
              </div>
            </div>
          )}
