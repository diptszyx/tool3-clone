...
export async function createTokenTransferTx(
  ...
  if (mintAAddress === NATIVE_SOL) {
    ...
    if (userBalance < amountANum) {
      throw new Error(
        `Insufficient Native SOL balance for token A. Required: ${amountANum / 1_000_000_000} SOL`,
      );
    }
  } else {
  ...
  if (mintBAddress === NATIVE_SOL) {
    ...
    if (userBalance < requiredLamports) {
      throw new Error(
        `Insufficient Native SOL balance for token B. Required: ${requiredLamports / 1_000_000_000} SOL`,
      );
    }
  } else {
