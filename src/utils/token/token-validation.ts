...
export function validateBasicTokenData(
  tokenData: {
    name: string;
    symbol: string;
...
export const requiredExtensionFields: Record<string, ExtensionField[]> = {
  ...
  'permanent-delegate': [{ id: 'delegate-address', label: 'Delegate Address', required: true }],
  'mint-close-authority': [
    { id: 'close-authority', label: 'Close Authority Address', required: true },
  ],
  'transfer-hook': [{ id: 'program-id', label: 'Program ID', required: true }],
};
...
export function checkExtensionRequiredFields(
  extensions: string[],
    ...
    for (const field of requiredFields) {
      if (field.required) {
        ...
        if (!value || (typeof value === 'string' && value.trim() === '')) {
          ...
          (field.id === 'fee-receiver' ||
            field.id === 'delegate-address' ||
            field.id === 'close-authority' ||
            field.id === 'program-id') &&
          typeof value === 'string'
        ) {
