...
export const transferToken = async (
  ...
  try {
    ...
    try {
      const sourceAssociatedAddress = await token.getAssociatedAddress(publicKey, false);
      ...
      throw new Error(
        'Source token account not found. You need to have tokens in your wallet before transferring.',
      );
    }
    ...
    if (error instanceof Error) {
      if (error.message?.includes('NonTransferable')) {
        ...
        errorMessage =
          'Token account not found. Please make sure you have enough SOL to create token account';
      }
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message?.includes('NonTransferable')) {
        ...
        errorMessage =
          'Token account not found. Please make sure you have enough SOL to create token accounts';
      }
