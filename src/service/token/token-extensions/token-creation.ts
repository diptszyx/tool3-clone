...
import {
  validateBasicTokenData,
  checkExtensionRequiredFields,
} from '@/utils/token/token-validation';
...
export function useTokenCreation() {
    ...
    if (selectedExtensions.includes(extensionId)) {
      if (isMetadataType) {
        setSelectedExtensions((prev) =>
          prev.filter((id) => id !== 'metadata' && id !== 'metadata-pointer'),
        );
        if (activeTab === 'metadata' || activeTab === 'metadata-pointer') {
          const remainingExtensions = selectedExtensions.filter(
            (id) => id !== 'metadata' && id !== 'metadata-pointer',
          );
      ...
      if (!compatibility.compatible) {
        const incompatibleExt = tokenExtensions.find(
          (ext) => ext.id === compatibility.incompatibleWith,
        );
        toast.error(
          `${extension?.name} is not compatible with the selected ${incompatibleExt?.name}`,
        );
        return;
      }
      ...
      if (extensionId === 'transfer-fees') {
        ...
        if (transferFeeExt) {
          const feePercentageOption = transferFeeExt.options.find(
            (opt) => opt.id === 'fee-percentage',
          );
  ...
  const validateTokenData = (): boolean => {
    ...
    const extensionValidation = checkExtensionRequiredFields(
      selectedExtensions,
